...
import pgenmi_torch
from pgenmi_torch import get_file_dims, load_data, PGM_LL, estep, mstep, partition_data



def pgenmi_torch_final_run(indir= '../input', outdir= 'pgenmi_final_results', regul_= 10, dirc= 'up',
                           evid_type= 'TFBS_DiffMark', dist= '50Kb', perc= 0.8, delta= 0.001, is_H0= False,
                           mode= 'all', output_file= None):

    assert mode in ['train', 'all'], 'mode is not valid: %r' % mode
    #assert evid_type in ['TFBS_DiffACC', 'TFBS_DiffMark', 'TFBS_PresACC',
    ...
    i= 1
    while i<= min_steps or (delta_loglike > delta and i<=max_steps): # add delta later
    ...
    if mode == 'train': # needs to be adjusted (it is not used here so it is not adjusted)
        with torch.no_grad():
        ...
        np.savetxt(f'{out_path}/pgenmi_final_stat', params_merged, delimiter= '\t')
        return loglike_np
