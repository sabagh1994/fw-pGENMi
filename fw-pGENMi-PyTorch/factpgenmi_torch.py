...
class PGM_LL_modif(nn.Module):
    ...
    def forward(self, x):
        pgm_w= (self.pgm_w_tf@self.pgm_w_m).reshape(-1,1)
        pgm_w= torch.cat((self.pgm_bias, pgm_w), dim= 0)
        y= x@pgm_w
        return y, pgm_w    
...
def estep(model, x, pg, regul=0, is_H0= False):
    inp, pgm_w= model(x)
    prob_zg1= torch.sigmoid(torch.squeeze(inp)) #17511 is the shape
    a= model.alpha
    ...
    if not is_H0:
        L2_loss= torch.sum(torch.dot(pgm_w[1:,:].reshape(-1), pgm_w[1:,:].reshape(-1)))
    else:
        L2_loss= 0
    ...
    return loglik_org, loglik, new_a, post_zg1, pgm_w
